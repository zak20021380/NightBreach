...
import {
  ...
  zombieApproachSlotUsage,
  ZOMBIE_SPAWN_FALLBACK_POSITIONS,
  ZOMBIE_SPAWN_POSITIONS,
  ZOMBIE_WAVE_CONFIG,
  type ZombieHitZoneType,
  type ZombieVisualFactory,
} from './game/zombies/zombieConfig'
import {
  createGlbZombieFactory,
  createProceduralZombieFactory,
} from './game/zombies/zombieFactory'
import {
  Zombie,
  type ZombieContext,
  type ZombieHitZone,
} from './game/zombies/zombie'
...
function getZombieVisualFactory() {
  ...
  zombieFactoryPromise = (async () => {
    ...
    if (result.status === 'fallback') {
      ...
      return createProceduralZombieFactory({ scene, createMaterial })
    }
...
// Explicit dependency bundle passed to every Zombie instance, replacing the
// module-level globals the class previously closed over.
const zombieContext: ZombieContext = {
  scene,
  zombies,
  hitZones: zombieHitZones,
  hitZoneMaterial: zombieHitZoneMaterial,
  damagePlayer,
  onZombieDied,
  onZombieDisposed: unregisterActiveZombie,
}
...
async function initializeZombies() {
  ...
  try {
    ...
    factory = createProceduralZombieFactory({ scene, createMaterial })
    markProceduralZombieSource()
  }
